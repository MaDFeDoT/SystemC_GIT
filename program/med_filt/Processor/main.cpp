...
int sc_main(int ac, char *av[])
{
  ...
  /*FILE *save_data = fopen("dcache","w");
	for (int kol=0; kol<1000; kol++) {
		fprintf(save_data, "0x%08x\n", 0 + rand() % 10000);
	}*/
  //Generate data for matrix
	/*FILE *save_data = fopen("dcache","w");
	int num=0;
	for (int kol=0; kol<121; kol++) {
		if(kol<150)
			fprintf(save_data, "0x%08x\n", num);
		else
			fprintf(save_data, "0xdeadbeef\n");
		num++;
		
	}*/
	/*for (int kol=0; kol<400; kol++) {
		if(kol<135)
			fprintf(save_data, "0x%08x\n", 1);
		else
			fprintf(save_data, "0xdeadbeef\n");
